...
export default function GraduationOverlay({ token, plan, onDone }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`${token.ticker} graduou`}
      className={cx(
    ...
    >
      ...
      <div className="relative max-w-[440px] w-full">
        ...
        >
          ${token.ticker} GRADUOU
        </h2>
        <p className="text-cream-50/80 mt-2">Acendeu a fogueira grande.</p>
        ...
        >
          ...
          <dl className="num text-caption text-cocoa-800 mt-3 space-y-1">
            ...
            <div className="flex justify-between gap-3">
              <dt>Liquidez</dt><dd>{money(plan?.quote_liquidity || token.raised, token.pair)} + 200M ${token.ticker}</dd>
            </div>
            <div className="flex justify-between gap-3">
        ...
        <p className="text-caption text-cream-50/50 mt-6">toque ou Esc para pular</p>
      </div>
    </div>
  );
}
